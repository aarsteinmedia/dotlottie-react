const files = ['1000.json',
  '1005.json',
  '1016.json',
  '1582.json',
  '1729.json',
  '2012.json',
  '2091.json',
  '2183.json',
  '3013.json',
  '3018.json',
  '32b1-d979.lottie',
  '4001.json',
  '4011.json',
  '4013.json',
  '404.lottie',
  '404_bulb.lottie',
  '6142.json',
  '6142.lottie',
  '6468.json',
  '6538.json',
  '6555.json',
  '6555.lottie',
  '6555_.json',
  '6558.json',
  '6561.json',
  'BuildGarden copy.lottie',
  'BuildGarden.lottie',
  'BuildOwnGarden.lottie',
  'BuildYourOwnGarden.lottie',
  'Network-Encryption.lottie',
  'West Mini Hero.json',
  'am.lottie',
  'apple-tv.lottie',
  'aroundTheWorld.json',
  'aroundTheWorld.lottie',
  'aroundTheWorld_.json',
  'b2b.lottie',
  'blackCat.json',
  'blackCat.lottie',
  'bodymovin.json',
  'bulb.lottie',
  'card-stack.lottie',
  'care-education-1.lottie',
  'care-education-2.lottie',
  'care-education-combined.lottie',
  'cat.json',
  'dev.lottie',
  'dinide.lottie',
  'factoryLine.json',
  'factoryLine.lottie',
  'gameAnimation.lottie',
  'gradient-stroke.json',
  'hero-web.json',
  'hero-web.lottie',
  'home_idea_es.lottie',
  'hp-section-2.lottie',
  'intro.json',
  'laptop.lottie',
  'loading.json',
  'loopOut-star.json',
  'loopOut-star.lottie',
  'lottieFiles.json',
  'manually-combined.json',
  'menuAnimation-intro.lottie',
  'menuAnimation.json',
  'menuAnimation.lottie',
  'min-transform.json',
  'minihero.lottie',
  'models.lottie',
  'optimized_m2acyje2.lottie',
  'portal.lottie',
  'road-trip.lottie',
  'rond-qui-tourne.json',
  'rond-qui-tourne.lottie',
  'rond-qui-tourne_.lottie',
  'round-corners-modifier.json',
  'round-corners-modifier_.json',
  'seedRandom.json',
  'shapes-test.json',
  'strengths.json',
  'strengths.lottie',
  'synnove.lottie',
  'text.json',
  'think.json',
  'think.lottie',
  'thinking.json',
  'thinking.lottie',
  'tysk.json',
  'ux.json',
  'ux.lottie',
  'waving_thinking.lottie',
  'wine.lottie',
  'wp-banner-pro.json',
  'wp-banner-pro.lottie',
  'wp-banner.json',
  'wp-banner.lottie']

export default files